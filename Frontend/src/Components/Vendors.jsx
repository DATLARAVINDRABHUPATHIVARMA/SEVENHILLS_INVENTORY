    ...
    if (editVendor) {
      try {
        const response = await axios.put(
          `http://localhost:3000/api/vendor/${editVendor}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("store-token")}`,
      ...
      try {
        const response = await axios.post(
          "http://localhost:3000/api/vendor/add",
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("store-token")}`,
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4 p-4">
      ...
      <div className="flex justify-between items-center">
        <input
          type="text"
          placeholder="Search By Vendor ID / Name"
            ...
            <tbody>
              {filteredVendors.map((vendor, index) => (
                <tr key={vendor._id}>
                  ...
                  <td className="border border-gray-300 p-2">
                    <button
                      className="px-2 py-1 bg-yellow-500 text-white rounded cursor-pointer hover:bg-yellow-600 mr-2"
